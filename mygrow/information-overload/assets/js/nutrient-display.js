...
try {
    ...
    console.error('Failed to load nutrient-data.json:', error);
}
    ...
    try {
        ...
        if (scheduleRequest.result && Array.isArray(scheduleRequest.result.schedule)) {
            ...
            if (modified) {
                ...
                console.log('Added stage-specific nutrients to schedule days', selectedNutrients, schedule.slice(0, 5));
            }
        }
    } catch (error) {
        console.error('Error handling nutrients:', error);
    }
});