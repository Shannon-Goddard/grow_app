...
async function loadAndRenderTable(growId) {
    try {
        ...
        for (let i = 0; i < schedule.length; i += chunkSize) {
            ...
            row.dataset.stage = dayData.stage || '';
...
async function populateDropdown(growId) {
    ...
    grows.forEach(g => {
        ...
        opt.value = g.growId;
        opt.textContent = localStorage.getItem(`growName_${g.growId}`) || g.growId;
        sel.appendChild(opt);
    });
    if (growId) sel.value = growId;
    sel.addEventListener('change', async () => {
        const id = sel.value;
        localStorage.setItem('currentGrowId', id);
        showGrowInfo(id);
        await loadAndRenderTable(id);
    });
}

function showGrowInfo(gId) {
    const bar = document.getElementById('growInfoBar');
    if (!bar) return;
    document.getElementById('growNameDisplay').textContent = localStorage.getItem(`growName_${gId}`) || 'My Grow';
    document.getElementById('growMetaDisplay').textContent = localStorage.getItem(`plantStrain_${gId}`) || '';
    const logo = localStorage.getItem(`plantLogo_${gId}`) || '';
    const img = document.getElementById('growLogo');
    if (img && logo) {
        img.src = 'assets/strain-img/' + logo.split('/').pop();
        img.onerror = () => { img.src = 'assets/strain-img/default.jpg'; };
    }
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const growId = params.get('growId') || localStorage.getItem('currentGrowId');
    if (growId) localStorage.setItem('currentGrowId', growId);
    await populateDropdown(growId);
    if (growId) {
        showGrowInfo(growId);
