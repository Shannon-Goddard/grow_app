...
import { getPlantGrow } from './storage.js';
...
const prepareSchedule = async (growId, startDate, isAuto, seedToHarvestDays, vegWeeks) => {
    try {
        const scheduleResponse = await fetch('/medium-feeding/assets/data/schedule-data.json');
        if (!scheduleResponse.ok) throw new Error('Failed to load schedule-data.json');
        const scheduleData = await scheduleResponse.json();

        const nutrientResponse = await fetch('/medium-feeding/assets/data/nutrient-data.json');
...
async function saveScheduleToIndexedDB(growId, startDate, isAuto, seedToHarvestDays, vegWeeks) {
    try {
        console.log('[saveSchedule] start', { growId, startDate, isAuto, seedToHarvestDays, vegWeeks });
        const schedule = await prepareSchedule(growId, startDate, isAuto, seedToHarvestDays, vegWeeks);
        console.log('[saveSchedule] prepareSchedule result:', schedule ? schedule.length + ' rows' : 'NULL');
        if (!schedule) return false;
        console.log('[saveSchedule] row[0]:', JSON.stringify(schedule[0]));
        const result = await IndexedDBService.saveSchedule(growId, schedule);
        console.log('[saveSchedule] saveSchedule result:', result);
        if (result) localStorage.setItem(`scheduleSaved_${growId}`, 'true');
        return result;
    } catch (error) {
