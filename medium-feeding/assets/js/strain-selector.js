...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function populateList(strains, originalIndices) {
        ...
        strains.forEach((strain, displayIndex) => {
            ...
            const li = $(`
                <li>
                    <img loading="lazy" class="lozad" data-src="${strain.logo}" width="40" height="40" alt="${strain.strain}">
            ...
            li.find('img').on('error', function () {
                $(this).attr('src', 'assets/strain-img/default.jpg');
            });
    ...
    $('#addStrainForm').on('submit', async function (e) {
        ...
        // Show loading animation only after validation passes
        document.body.classList.add('page-transitioning');

        const strainName = $('#customStrainName').val().trim();
        const isAuto = $('input[name="strainType"]:checked').val() === 'auto';
        
        // Track custom strain creation in Google Analytics
        if (typeof gtag === 'function') {
            gtag('event', 'custom_strain_created', {
                'event_category': 'Grow Setup',
                'event_label': strainName,
                'strain_type': isAuto ? 'auto' : 'photoperiod'
            });
        }
        ...
        const randomLogo = `assets/strain-img/default-${randomLogoNum}.png`;
...
try {
    ...
    // Error saving grow to IndexedDB
}

        $('#addStrainModal').css('display', 'none');
        window.dispatchEvent(new CustomEvent('strainSelected'));
    });
    ...
    function displayStrainDetails(index) {
        ...
        try {
            ...
            const updates = {
                ...
                '#taskButton': { text: 'Select This Strain →' }
            };
    ...
    $('#taskButton').on('click', async function (e) {
        ...
        const logoSrc = displayedImgSrc && displayedImgSrc !== '' ? 
            displayedImgSrc : 
            `assets/strain-img/${strain.replace(/\s+/g, '_')}.jpg`;
        ...
        // Track strain selection in Google Analytics
        if (typeof gtag === 'function') {
            gtag('event', 'strain_selected', {
                'event_category': 'Grow Setup',
                'event_label': strain,
                'strain_type': isAuto ? 'auto' : 'photoperiod',
                'is_predefined': strainData ? 'yes' : 'no'
            });
        }
