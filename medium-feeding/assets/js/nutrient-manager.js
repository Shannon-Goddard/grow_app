...
function generateSchedule(nutrients, growId) {
    ...
    const floweringWeeks = grow.floweringWeeks || 16;
    const schedule = [];
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        request.onsuccess = () => {
            ...
            setTimeout(() => {
                document.querySelectorAll('.row .col-xl-2 .icon-box').forEach(section => {
                    section.classList.add('force-visible');
                    if (section.parentElement) section.parentElement.classList.add('force-visible');
                });
            }, 100);

            const staticCheckboxes = document.querySelectorAll('.listPrint input[type="checkbox"]');
            staticCheckboxes.forEach(checkbox => {
                if (!checkbox.value) checkbox.value = checkbox.id;
                checkbox.addEventListener('change', async () => {
                    await updateSelectedNutrients();
                    updateTaskButtonVisibility();
                });
            ...
            if (existingGrowNutrients) {
                JSON.parse(existingGrowNutrients).forEach(nutrient => {
...
export { updateSelectedNutrients, updateTaskButtonVisibility };
