...
class IndexedDBService {
    ...
    static getInstance(dbName = 'MyGrowDB', dbVersion = 8) {
        if (!instance) instance = new IndexedDBService(dbName, dbVersion);
        return instance;
    }
    ...
    initDB() {
        ...
        dbPromise = new Promise((resolve, reject) => {
            ...
            request.onupgradeneeded = (event) => {
                ...
                if (!existingStores.includes('tables')) db.createObjectStore('tables');
                if (!existingStores.includes('selectedNutrients')) db.createObjectStore('selectedNutrients', { keyPath: 'id' });
                if (!existingStores.includes('nutrients')) db.createObjectStore('nutrients', { keyPath: 'nutrientName' });
            };
    ...
    async saveSchedule(growId, schedule) {
        try {
            console.log('[IDB.saveSchedule] opening db for growId:', growId, 'rows:', schedule.length);
            const db = await this.initDB();
            console.log('[IDB.saveSchedule] db open, starting transaction');
            const transaction = db.transaction(['tables'], 'readwrite');
            const store = transaction.objectStore('tables');
            const request = store.put(schedule, `${growId}_schedule`);
            return new Promise((resolve, reject) => {
                request.onsuccess = () => { console.log('[IDB.saveSchedule] put success'); resolve(true); };
                request.onerror = (event) => { console.error('[IDB.saveSchedule] put error', event.target.error); reject(event.target.error); };
                transaction.onerror = (event) => { console.error('[IDB.saveSchedule] tx error', event.target.error); reject(event.target.error); };
            });
        } catch (error) {
            console.error('[IDB.saveSchedule] catch:', error);
            return false;
        }
    }
